let bars = [];
const def = "#91f8f6", chng = "#431f91", finished = "#8ef511", selected = "yellow";
...
function generateBars(n = -1) {
    ...
    for (let i = 0; i < n; i++) {
        ...
        bars.push(`<div class="bar" id="${i}" style="height:${height}%;"><span>${height}</span></div>`);
    }
...
async function SelectionSort() {
    ...
    document.getElementById("tc").innerText = "O(n^2)";
...
async function MergeSort() {
    document.getElementById("tc").innerText = "O(n*log(n))";
	let delay = Disable_The_Input();
